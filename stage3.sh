...
if [[ -d "$baseDir" ]]; then
        for dir in "$baseDir"/*/; do
                ...
                if [[ -f "${dir}/rootdomain.txt" ]]; then
                        ...
                        httpx -l "${dir}/resolved.txt" -sc -cl -favicon -title -ct -location -server -td -method -ip -cname -asn -cdn > "${dir}/metadata.txt"                      
                        
                        echo "Separating subs by status code..."
                        # Using sed to remove the color output in metadata file so the grep doesn't freak out later on
